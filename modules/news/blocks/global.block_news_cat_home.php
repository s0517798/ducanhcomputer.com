...
if (!nv_function_exists('nv_block_news_cat_home')) {
    /**
     ...
     * @return
     */
    function nv_block_config_news_cat_home($module, $data_block, $lang_block)
    {
     ...
     * @return
     */
    function nv_block_config_news_cat_home_submit($module, $lang_block)
    {
        global $nv_Request;
        $return = array();
        $return['error'] = array();
     ...
     * @return
     */
    function nv_block_news_cat_home($block_config)
    {
        ...
        $db->sqlreset()
            ->select('id, catid, title, alias, hitstotal, homeimgfile, homeimgthumb, hometext, publtime, external_link')
        ...
        if (!empty($list)) {
            if (file_exists(NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/news/block_groups_home.tpl')) {
                $block_theme = $global_config['module_theme'];
            } else {
                $block_theme = 'default';
            }

            $xtpl = new XTemplate('block_groups_home.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/news');
...
if (defined('NV_SYSTEM')) {
    ...
    if (isset($site_mods[$module])) {
        ...
        $content = nv_block_news_cat_home($block_config);
    }
}