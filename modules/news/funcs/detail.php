...
if (!empty($news_contents)) {
    ...
    $base_url_rewrite_old = nv_url_rewrite(NV_BASE_SITEURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&' . NV_OP_VARIABLE . '=' . $global_array_cat[$news_contents['catid']]['alias'] . '/' . $news_contents['alias'] . '-' . $news_contents['id'] . $global_config['rewrite_exturl'], true);
        ...
        if (!empty($news_contents['files'])) {
            ...
            foreach ($files as $file_id => $file) {
                ...
                $news_contents['files'][] = [
                    ...
                    'url' => (!preg_match("/^http*/", $file)) ? $base_url_rewrite_old . '?download=1&amp;id=' . $file_id : $file,
                    'urlpdf' => $base_url_rewrite_old . '?pdf=1&amp;id=' . $file_id,
