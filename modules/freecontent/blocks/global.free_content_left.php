...
if (!nv_function_exists('nv_block_freecontent_left')) {
    /**
     ...
     * @return
     */
    function nv_block_config_freecontent_left($module, $data_block, $lang_block)
    {
     ...
     * @return
     */
    function nv_block_config_freecontent_left_submit($module, $lang_block)
    {
        global $nv_Request;
        $return = array();
        $return['error'] = array();
     ...
     * @return
     */
    function nv_block_freecontent_left($block_config)
    {
        ...
        $sql = 'SELECT id, title, icon, description, image, link, target FROM ' . NV_PREFIXLANG . '_' . $site_mods[$module]['module_data'] . '_rows WHERE status = 1 AND bid = ' . $block_config['blockid'];
        $list = $nv_Cache->db($sql, 'id', $module);

        if (!empty($list)) {
            if (file_exists(NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/' . $site_mods[$module]['module_file'] . '/block.free_content_left.tpl')) {
                $block_theme = $global_config['module_theme'];
            } elseif (file_exists(NV_ROOTDIR . '/themes/' . $global_config['site_theme'] . '/modules/' . $site_mods[$module]['module_file'] . '/block.free_content_left.tpl')) {
                $block_theme = $global_config['site_theme'];
            } else {
                $block_theme = 'default';
            }

            $xtpl = new XTemplate('block.free_content_left.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/' . $site_mods[$module]['module_file']);
...
if (defined('NV_SYSTEM')) {
    $content = nv_block_freecontent_left($block_config);
}
