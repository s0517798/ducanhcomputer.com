...
{
    global $db, $global_config, $site_mods, $lang_module, $lang_global, $module_info, $module_name, $my_head, $download_config, $list_cats;
    ...
    foreach ($array_cats as $cat) {
        if (empty($cat['parentid'])) {
            ...
            foreach ($items as $item) {
				...
				$sql1 = 'SELECT linkdirect FROM ' . NV_MOD_TABLE . '_detail WHERE id =' . $item['id'];
				$linkdirect = $db->query($sql1)->fetchColumn();
				$item['linkdirect'] = $linkdirect;
...
{
    global $db, $global_config, $site_mods, $lang_module, $lang_global, $module_info, $module_name, $my_head, $download_config;
    ...
    if (!empty($array_files)) {
        foreach ($array_files as $file) {
			$sql1 = 'SELECT linkdirect FROM ' . NV_MOD_TABLE . '_detail WHERE id =' . $file['id'];
			$linkdirect = $db->query($sql1)->fetchColumn();
			$file['linkdirect'] = $linkdirect;
