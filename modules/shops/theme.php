    ...
    if ($pro_config['home_data'] == 'all') {
        ...
        if (!empty($array_data)) {
            foreach ($array_data as $data_row) {
				...
				if ($data_row ['in_stock'] == 1) {
					$data_row ['product_status'] = $lang_module ['product_is_available'];
					$data_row ['product_status_class'] = 'san-hang';
					$data_row ['product_available'] = 'InStock';
				} elseif ($data_row ['in_stock'] == 2) {
					$data_row ['product_status'] = $lang_module ['product_is_order'];
					$data_row ['product_status_class'] = 'dat-truoc';
					$data_row ['product_available'] = 'PreOrder';
				} elseif ($data_row ['in_stock'] == 3) {
					$data_row ['product_status'] = $lang_module ['product_is_out'];
					$data_row ['product_status_class'] = 'het-hang';
					$data_row ['product_available'] = 'OutOfStock';
				}
...
function view_home_cat($array_data, $compare_id, $pages = '', $sort = 0, $viewtype = 'viewgrid')
{
    global $module_info, $lang_module, $module_name, $module_file, $pro_config, $array_wishlist_id, $global_array_shops_cat, $global_array_group, $my_head;
...
{
    global $db, $db_config, $module_info, $lang_module, $module_file, $module_name, $module_upload, $pro_config, $global_config, $global_array_group, $array_wishlist_id, $client_info, $global_array_shops_cat, $meta_property, $pro_config, $user_info, $discounts_config, $my_head, $my_footer;
    ...
    if (!empty($data_content)) {
        ...
        $xtpl->assign('DATE_UP', nv_date('Y-m-d', $data_content['publtime']));
		...
		$xtpl->assign('product_status_class', $data_content['product_status_class']);
        $xtpl->assign('product_status', $data_content['product_status']);
        $xtpl->assign('pro_unit', $data_unit['title']);
		//rating
		$db->sqlreset()
		  ->select('rating, sender')
		  ->from($db_config['prefix'] . "_" . $module_info['module_data'] . "_review")
		  ->where("product_id = " . $data_content['id'])
		  ->order("add_time DESC");
		...
		$db->sqlreset()
		  ...
		  ->where("product_id = " . $data_content['id'])
		  ->order("add_time DESC");
		$review1 = $db->query($db->sql())->fetchAll();
		$dem=0;
		$rating=0;
		foreach($review1 as $rw)
		{
			$rating += $rw['rating'];
			$dem++;
		}
		$xtpl->assign('RATINGTB', $rating/$dem);
		///end rating
