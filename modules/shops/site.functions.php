...
function nv_get_price($pro_id, $currency_convert, $number = 1, $per_pro = false, $module = '')
{
	...
	$return['money_unit'] = $product['money_unit'];
	if($product['money_unit'] == 'VND'){
		$return['unit'] = '₫';
	}
	else{
		$return['unit'] = $product['money_unit'];
	}
	$return ['giamgia'] = nv_number_format(100-($return['sale']*100/$return['price']),0);
    return $return;
}
