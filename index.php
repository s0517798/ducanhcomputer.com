...
if (preg_match($global_config['check_module'], $module_name)) {
	...
	if (!isset($site_mods[$module_name]) and !empty($alias_mod))
	{
		...
		if($array_alias['id'])
		{
			$global_parent_id_shop = $db->query(" SELECT parentid FROM " .  $db_config['prefix'] . '_' . $site_mods['san-pham']['module_data'] . "_catalogs WHERE catid = " . $array_alias['catid_alias'])->fetchColumn();
			if(!empty($global_parent_id_shop))
			{
				$global_parent_shop = $db->query("SELECT catid, vi_title AS title, vi_alias AS link FROM " .  $db_config['prefix'] . '_' . $site_mods['san-pham']['module_data'] . "_catalogs WHERE catid = " . $global_parent_id_shop)->fetchAll();
			}

			$global_title_shop = $db->query(" SELECT vi_title FROM " .  $db_config['prefix'] . '_' . $site_mods['san-pham']['module_data'] . "_catalogs WHERE catid = " . $array_alias['catid_alias'])->fetchColumn();			
			$global_title_id_shop = $db->query(" SELECT catid FROM " .  $db_config['prefix'] . '_' . $site_mods['san-pham']['module_data'] . "_catalogs WHERE catid = " . $array_alias['catid_alias'])->fetchColumn();			
			
			$global_link_shop = $db->query(" SELECT vi_alias FROM " .  $db_config['prefix'] . '_' . $site_mods['san-pham']['module_data'] . "_catalogs WHERE catid = " . $array_alias['catid_alias'])->fetchColumn();
			
			////////////////////////////////////
			$global_parent_id_news = $db->query(" SELECT parentid FROM " . NV_PREFIXLANG . "_" . $site_mods['news']['module_data'] . "_cat WHERE catid = " . $array_alias['catid_alias'])->fetchColumn();
			if(!empty($global_parent_id_news))
			{
				$global_parent_news = $db->query("SELECT catid, title, alias AS link FROM " . NV_PREFIXLANG . "_" . $site_mods['news']['module_data'] . "_cat WHERE catid = " . $global_parent_id_shop)->fetchAll();
			}
			$global_title_news = $db->query(" SELECT title FROM " . NV_PREFIXLANG . "_" . $site_mods['news']['module_data'] . "_cat WHERE catid = " . $array_alias['catid_alias'])->fetchColumn();			
			$global_title_id_news = $db->query(" SELECT catid FROM " . NV_PREFIXLANG . "_" . $site_mods['news']['module_data'] . "_cat WHERE catid = " . $array_alias['catid_alias'])->fetchColumn();			
			
			$global_link_news = $db->query(" SELECT alias FROM " . NV_PREFIXLANG . "_" . $site_mods['news']['module_data'] . "_cat WHERE catid = " . $array_alias['catid_alias'])->fetchColumn();			
